...
def rem(inputfile,outfile):
    ...
    patient_ids = set()
    patient_ids_2012 = set()
    patient_ids_2013 = set()
    for line in filehandle:
        values = line.strip().split(",")

        if values[SRC_COL_IDX] == '1' and values[ADMIT_YEAR_IDX] == '2011':
            patient = values[32]
    ...
    visit_dist_2013 = defaultdict(int)
    ...
    for line in filehandle:
        ...
        if prev_id != patient:
            ...
            update_visit_distibution(visit_dist_2013, per_year_visit_cnt['2013'])
            visit = []
        ...
        elif values[ADMIT_YEAR_IDX] == '2012':
            visit.append('2012')
            w_fhand.write(line)
            patient_ids_2012.add(patient)

        elif values[ADMIT_YEAR_IDX] == '2013':
            visit.append('2013')
            w_fhand.write(line)
            patient_ids_2013.add(patient)
        prev_id = patient

    filehandle.close()
    w_fhand.close()
    print len(patient_ids), len(patient_ids_2012), len(patient_ids_2013)
    print visit_dist_2011,  visit_dist_2012, visit_dist_2011

rem('/Users/oshpddata/Desktop/OSHPD2016/OSHPD_ALLCAUSE.csv','/Users/oshpddata/Desktop/vikhyati/2011_ED_FORWARD.csv' )
